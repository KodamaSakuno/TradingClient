...
/// </summary>
public sealed record HedgeAdvice(HedgeDirection Direction, int Lots, double NetDelta)
{
    public string Text => Direction switch
    {
        HedgeDirection.ShortFutures => $"净 Delta {NetDelta:F2}（吨）→ 建议做空 {Lots} 手标的期货对冲",
        HedgeDirection.LongFutures => $"净 Delta {NetDelta:F2}（吨）→ 建议做多 {Lots} 手标的期货对冲",
